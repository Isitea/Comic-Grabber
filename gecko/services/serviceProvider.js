...
async function main () {
    ...
    async function communication ( { message, action, clientUid, data }, sender ) {
        let result = undefined;
        switch ( action ) {
            case constant.__request__: {
                result = await fetch( data.uri ).then( body => body.text() );
            }
        ...
        return { action, clientUid, data: result };
        $client.tabs.sendMessage( sender.tab.id, { action, clientUid, data: result } );
    }
    $client.runtime.onMessage.addListener( communication );
