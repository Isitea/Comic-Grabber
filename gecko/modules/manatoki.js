...
let pageModule = async () => new Promise( resolve => {
    try {
        ...
        let images = Promise.resolve( [ ...document.querySelectorAll( `div.${altId} img` ) ].map( item => item.dataset[id] ) );
        let raw = document.querySelector( `.toon-info .toon-title` ).firstChild.textContent.toFilename();
